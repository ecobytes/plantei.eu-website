...
$('document').ready(function() {
    ...
    function resetModalFormErrors() {
        $('.body-form .validationErrors').empty();
    } 

    // Intercept submit.
    $('.modal').on('submit', 'form.bootstrap-modal-form', function(submission) {
        ...
        // Response.
            ...
            if (response.status == 422) {
                ...
                $('.body-form').prepend('<div class="row validationErrors"></div>');


                // Iterate through errors object.
                $.each(errors, function(field, message) {
                    console.error(field+': '+message);
                    $('.body-form .validationErrors').append('<div class="alert alert-danger col-sm-6 col-sm-offset-3" role="alert">' + message + '</div>');

                    /*var formGroup = $('[name='+field+']', form).closest('.form-group span');
